...
public class JHTTPPAdmin {
	...
	private void getParams(String request)
	{
		boolean d = true;
		int prev = 0;
	...
	private boolean parseParam(String pair, Properties p)
	{
	...
	public String HTMLAdmin() {
		...
		if (server.config_auth == 0) {
			...
			msg = msg
					...
					// server.debug)
					+ chk_option("server.debug-logging", "debug", server.isDebugEnabled())
		...
		msg = msg
				// + "<HR size=\"4\">\r"
				+ "<p class=\"i25\"><A HREF=\"http://www.zoxweb.com/\">Java Proxy</A> HTTP Proxy, Version "
				+ server.getServerVersion()
				+ "\r"
				+ " webadmin v."
				+ JHTTP2ADMIN_VERSION
				+ "\r"
				+ "<br>Copyright &copy; 2001-2003 <A HREF=\"mailto:support@zoxweb.com\">ZoxWeb Support</A></p>\r"
	...
	public static String HTMLFiltersList() {
		...
		while (an.hasMoreElements()) {
			JHTTPPURLMatch a = (JHTTPPURLMatch) an.nextElement();
